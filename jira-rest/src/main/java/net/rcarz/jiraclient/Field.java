...
public final class Field {
  ...
  public static List<AllowedValue> getResourceArray(Class<AllowedValue> type, Object ra) {
    List<AllowedValue> results = new ArrayList<>();
    if (ra instanceof JSONArray) {
      for (Object v : (JSONArray) ra) {
          JSONObject object = (JSONObject) v;
          AllowedValue value = new AllowedValue();
        if (object.containsKey("value")) value.value = object.getString("value");
        if(object.containsKey("name")) value.value = object.getString("name");
          value.id = object.getString("id");
          value.self = object.getString("self");
          results.add(value);
      }
    }
    return results;
    ...
    public static Meta getFieldMetadata(String name, JSONObject editmeta)
        ...
        System.out.println(editmeta.get(name));
    ...
    if (f.containsKey("allowedValues")) {
      m.allowedValues = Field.getResourceArray(AllowedValue.class, f.get("allowedValues"));
}
        return m;
    }
     ...
     */
    public static JSONArray toArray(Iterable iter, String type, String custom,List<AllowedValue>allowedValues) throws JiraException {
        ...
        for (Object val : iter) {
            ...
            if (type.equals("component") || type.equals("group") ||
                ...
                Object finalValue = realValue;
                AllowedValue allowedValue1 = allowedValues.stream().filter(allowedValue -> allowedValue.value.equals(finalValue.toString()))
                        .findFirst().orElseThrow(() -> new JiraException(String.format("Failed to find allowed value for value -> %s",finalValue.toString() )));
                ((JSONObject)realResult).put("id",allowedValue1.id);
                ((JSONObject)realResult).put("value",allowedValue1.value);
                ((JSONObject)realResult).put("self",allowedValue1.self);
    ...
    public static Object toJson(String name, Object value, JSONObject editmeta)
        ...
        if(m.system != null && m.system.equals("summary")){
        ...
        if (m.type.equals("array")) {
            ...
            return toArray((Iterable)value, m.items, m.custom,m.allowedValues);
            ...
            if(m.allowedValues.isEmpty()){
                return value.toString();
            } else {
                Object finalValue = value;
                AllowedValue allowedValue1 = m.allowedValues.stream().filter(allowedValue -> allowedValue.value.equals(finalValue.toString()))
                        .findFirst().orElseThrow(() -> new JiraException(String.format("Failed to find allowed value for value -> %s",finalValue.toString() )));
                mainJSON.put("id",allowedValue1.id);
                mainJSON.put("value",allowedValue1.value);
                mainJSON.put("self",allowedValue1.self);
                return mainJSON;
            }
