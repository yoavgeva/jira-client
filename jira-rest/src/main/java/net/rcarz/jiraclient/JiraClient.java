...
public class JiraClient {
  ...
  public List<String> requiredFieldsIds(String project, String issueTypeName) throws JiraException {
    JSONObject createMetadata = Issue.getCreateMetadata(getRestClient(), project, issueTypeName);
    List<String> ids = new ArrayList<>();
    Iterator<String> keys = createMetadata.keys();
    Set<String> invalidKeys = new HashSet<>();
    invalidKeys.add("issuetype");
    invalidKeys.add("project");
    invalidKeys.add("summary");
    invalidKeys.add("assignee");
    while (keys.hasNext()) {
      String key = keys.next();
      JSONObject object = createMetadata.getJSONObject(key);
      if (object.getBoolean("required") && !invalidKeys.contains(key)) {
        ids.add(key);
      }
    }
    return ids;
    ...
    /**
     * Obtains the list of all fields in Jira.
     * @return all fields; not all data is returned for each fields; to get
     * the extra data use {@link #getProject(String)}
     * @throws JiraException failed to obtain the project list.
     */
    public List<FieldJson> getFields() throws JiraException {
        try {
            URI uri = restclient.buildURI(Resource.getBaseUri() + "field");
            JSON response = restclient.get(uri);
            System.out.println(response.toString());
            JSONArray fieldsArray = JSONArray.fromObject(response);

            List<FieldJson> fields = new ArrayList<FieldJson>(fieldsArray.size());
//            for (int i = 0; i < fieldsArray.size(); i++) {
//                JSONObject p = fieldsArray.getJSONObject(i);
//                fields.add(new FieldJson(restclient, p));
//            }
